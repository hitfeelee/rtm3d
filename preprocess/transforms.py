...
class RandomAffine(object):
    def __init__(self, mean, range=0.5, offset=0.5):
        self.range = range
        self.offset = offset
        self.mean = [m * 255 for m in mean] if np.amax(mean) <= 1. else mean
    ...
    def __call__(self, image, targets=None, **kwargs):
        ...
        if targets.has_field('mask'):
            bboxes = targets.get_field('bbox')
            center_x, center_y = data_utils.bbox_center(bboxes).T
...
class RemoveBadBBox(object):
    def __call__(self, image, targets=None, **kwargs):
        if targets is not None and targets.has_field('mask'):
            mask = targets.get_field('mask')
            bbox = targets.get_field('bbox')
            w = bbox[:, 2] - bbox[:, 0]
            h = bbox[:, 3] - bbox[:, 1]
            remove = (w <= 2) | (h <= 2)
            mask[remove] = 0
...
class RandomBrightnessContrast(object):
    def __init__(self, p=0.5):
        self._trans = album.RandomBrightnessContrast(p=p)

    def __call__(self, image, targets=None, **kwargs):
        return self._trans(image=image)['image'], targets


class GaussNoise(object):
    def __init__(self, p=0.5):
        self._trans = album.GaussNoise(p=p)
