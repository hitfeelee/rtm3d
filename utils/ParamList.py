...
class ParamList(object):
    ...
    def delete_by_mask(self):
        if self.has_field('mask'):
            ...
            m = mask.clone() if isinstance(mask, torch.Tensor) else np.copy(mask)
            m = m > 0
            for k, v in self.extra_fields.items():
                if len(m) != len(v):
                    print('debug')
    ...
    def copy_field(self, other, fields):
        for f in fields:
            if other.has_field(f):
                self.add_field(f, copy.deepcopy(other.get_field(f)))
