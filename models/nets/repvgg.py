...
class RepVGG(nn.Module):
    ...
    def __init__(self, configs, num_blocks, width_multiplier=None, override_groups_map=None, deploy=False):
        ...
        channels = [int((2**(i + 6)) * width_multiplier[0]) for i in range(4)]
        self.layer0 = RepVGGBlock(in_channels=3, out_channels=self.in_planes, kernel_size=3, stride=2, padding=1, deploy=self.deploy)
        self.cur_layer_idx = 1
        self.layer1 = self._make_layer(channels[0], num_blocks[0], stride=2)
...
def create_RepVGG_A0(configs, deploy=False):
    return RepVGG(configs, num_blocks=[2, 4, 14, 1],
                  width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=None, deploy=deploy)

def create_RepVGG_A1(configs, deploy=False):
    return RepVGG(configs, num_blocks=[2, 4, 14, 1],
                  width_multiplier=[1, 1, 1, 2.5], override_groups_map=None, deploy=deploy)

def create_RepVGG_A2(configs, deploy=False):
    return RepVGG(configs, num_blocks=[2, 4, 14, 1],
                  width_multiplier=[1.5, 1.5, 1.5, 2.75], override_groups_map=None, deploy=deploy)

def create_RepVGG_B0(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[1, 1, 1, 2.5], override_groups_map=None, deploy=deploy)

def create_RepVGG_B1(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2, 2, 2, 4], override_groups_map=None, deploy=deploy)

def create_RepVGG_B1g2(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2, 2, 2, 4], override_groups_map=g2_map, deploy=deploy)

def create_RepVGG_B1g4(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2, 2, 2, 4], override_groups_map=g4_map, deploy=deploy)


def create_RepVGG_B2(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2.5, 2.5, 2.5, 5], override_groups_map=None, deploy=deploy)

def create_RepVGG_B2g2(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2.5, 2.5, 2.5, 5], override_groups_map=g2_map, deploy=deploy)

def create_RepVGG_B2g4(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[2.5, 2.5, 2.5, 5], override_groups_map=g4_map, deploy=deploy)


def create_RepVGG_B3(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[3, 3, 3, 5], override_groups_map=None, deploy=deploy)

def create_RepVGG_B3g2(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
                  width_multiplier=[3, 3, 3, 5], override_groups_map=g2_map, deploy=deploy)

def create_RepVGG_B3g4(configs, deploy=False):
    return RepVGG(configs, num_blocks=[4, 6, 16, 1],
...
#   Use like this:
#   train_model = create_RepVGG_A0(configs, deploy=False)
#   train train_model
#   deploy_model = repvgg_convert(train_model, create_RepVGG_A0, save_path='repvgg_deploy.pth')
def repvgg_model_convert(model:torch.nn.Module, build_func, configs, save_path=None):
    converted_weights = {}
    ...
    deploy_model = build_func(configs, deploy=True)
